...
void performImageProcessing(const string& inputPath, const string& outputPath) {
    ...
    std::cout << "Preliminary Time: "
              << measureRuntime(preliminarySharpened, h_pEdge, h_pError, h_upscaled, width, height, mean,
                                lightStrength)
              << " ms" << std::endl;
    ...
    std::cout << "Sharpened and upscaled image saved as " << outputPath << std::endl;
...
int main(int argc, char *argv[]) {
    ...
    // Gọi hàm xử lý ảnh
    performImageProcessing(inputPath, outputPath);
